...
/**
 * Reads/deserialize OGC WMC Documents.
 */
public class XmlBeansContextReader implements ContextReader {
    ...
    public LayerContext readFrom(BufferedReader reader) throws OXFException {
        ...
        ViewContextType xb_context = parseContextFromReader(reader);
        ...
        for (int i = 0; i < xb_layerList.sizeOfLayerArray(); i++) {
            ...
            if (layerType.equals("featureLayer")) {
                ...
                for (int j = 0; j < nodeList.getLength(); j++) {
                    ...
                    String className = currentNode.getNodeValue();
                    if (className.equals("serviceConnector")) {
                        adapter = createInstanceFrom(className);
                    }
                    else if (className.equals("renderer")) {
                        renderer = createInstanceFrom(className);
                    }
                    else if (className.equals("featureStore")) {
                        featureStore = createInstanceFrom(className);
                    }
                    else if (className.equals("featurePicker")) {
                        featurePicker = createInstanceFrom(className);
                    }
                }
    ...
    private <T> T createInstanceFrom(String className) throws OXFException {
        try {
            ClassLoader loader = XmlBeansContextReader.class.getClassLoader();
            Class<T> classInstance = (Class<T>) loader.loadClass(className);
            return classInstance.newInstance();
        } catch (IllegalAccessException e) {
            throw new OXFException("Illegal Access Modifier.", e);
        }
        catch (InstantiationException e) {
            throw new OXFException("Could not create instance for class '" + className + "'.");
        }
        catch (ClassNotFoundException e) {
            throw new OXFException("Could not find class for instantiation: " + className, e);
        }
    }

    private ViewContextType parseContextFromReader(BufferedReader reader) throws OXFException {
        try {
            return ViewContextType.Factory.parse(reader);
        }
        catch (XmlException e) {
            throw new OXFException("Could not parse context file.", e);
        }
        catch (IOException e) {
            throw new OXFException("Could not read from input stream.", e);
        }
    }
...
//  }
    ...
    public static void main(String[] args) throws XmlException, IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, OXFException {
        File file = new File("C:/temp/MyContext.xml");
        
        FileInputStream fis = new FileInputStream(file);
        BufferedReader reader = new BufferedReader(new InputStreamReader(fis));
        ContextReader fac = new XmlBeansContextReader();
