...
public class OXFFeature /*implements org.opengis.feature.Feature*/ {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(OXFFeature.class);
    ...
    public static OXFFeature createFrom(SFSpatialSamplingFeatureType samplingFeatureType) {
        ...
        if (hasGmlPoint32){
            ...
            } else if (samplingFeatureType.getSampledFeatureArray() != null && samplingFeatureType.getSampledFeatureArray().length > 0) {
                if (samplingFeatureType.getSampledFeatureArray()[0].getHref() != null) {
                    featureType = samplingFeatureType.getSampledFeatureArray()[0].getHref();
                    if (samplingFeatureType.getSampledFeatureArray().length > 1) {
                        LOGGER.warn("SF sampled feature contains more than one feature. Feature type is taken from the first.");
                    }
                }
            }
